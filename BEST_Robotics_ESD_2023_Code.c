...
task arm { // Arm movement.
	// snap action switches here
	if(vexRT[Btn8D]) { // and snap action switch is off
		motor[motorLift] = 127;
	} else if (vexRT[Btn8U]) { // and snap action switch is off
		motor[motorLift] = -127;
	} else {
		motor[motorLift] = 0;
	}
	if(vexRT[Btn7D]) { // and snap action switch is off
		motor[motorArm] = 127;
	} else if (vexRT[Btn7U]) { // and snap action switch is off
		motor[motorArm] = -127;
	} else {
		motor[motorArm] = 0;
	}
